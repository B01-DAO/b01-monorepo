...
function buildSegments(rocketGroup: Group, rocketMaterial: MeshPhysicalMaterial) {
    ...
    function buildSegment(bottomRadius: number, topRadius: number, height: number, material: Material, type = "") {
        let segment;
        if (type == "cone") {
    ...
    buildSegment(radius, radius, bandHeight, new THREE.MeshPhysicalMaterial({color: getRandomColorInTheme()})); // band
...
function buildFins(rocketGroup: Group, rocketMaterial: MeshPhysicalMaterial) {
    const finShape = new THREE.Shape([new THREE.Vector2(radius * 2, 0.5), new THREE.Vector2(radius * 2, 0), new THREE.Vector2(0, baseoffset + 0.2), new THREE.Vector2(0, baseoffset + 1)]);
